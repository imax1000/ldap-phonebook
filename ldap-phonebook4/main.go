...
// Person структура для хранения информации о человеке
type Person struct {
...
func createMainWindow() {
	...
	// Левая панель - дерево организаций и отделов
	...
	centerPanel, err := gtk.BoxNew(gtk.ORIENTATION_VERTICAL, 5)
	if err != nil {
	...
	centerPanel.PackStart(searchBox, false, false, 0)
	...
	centerPanel.PackStart(resultsScrolled, true, true, 0)

	// Правая панель - детальная информация
	rightPanel, err := gtk.BoxNew(gtk.ORIENTATION_VERTICAL, 5)
	if err != nil {
		fmt.Printf("Ошибка создания правой панели: %v\n", err)
		os.Exit(1)
	}
	...
	rightPanel.PackStart(detailsLabel, false, false, 0)
	...
	rightPanel.PackStart(detailsScrolled, true, true, 0)

	// Добавляем панели в главный контейнер
	mainBox.PackStart(leftPanel, false, true, 0)
	mainBox.PackStart(centerPanel, true, true, 0)
	mainBox.PackStart(rightPanel, false, true, 0)
...
func loadLDAPData() {
	...
	// Поиск организаций 		"(objectClass=organization)",
